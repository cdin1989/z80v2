...
; Using the ps/2 beyboard as input device and the vdp as output device. 
; Refercence:
...
NDISKS:	 	EQU	16                ; Number of disks in the system
...
BOOT:
    ...
    LD   C, KBC_LOCK_NUM
    CALL KBC_LockIndicator        ; Turn on NumLock indicator
;
...
;----------------------------------------------------------------------
CONOUT:
	CALL SIO_WriteChar            ; Output to ACIA
	CALL VDP_PutChar              ; Output to VDP
	RET
...
SELDSK:
	...
	LD 	 A, C
	CP 	 NDISKS                   ; Must be between 0 and 15
...
;-------------------------------------------------------------------
    ...
    LD   A, C
	RRCA
	RRCA                          ; Remove the low 2bit, since they are moved to the high 2bit of CF_REG3
...
DPBHD:
	...
	DB %11110000                  ; AL0: Dir BLK bitmap
	DB %00000000                  ; AL1: Dir BLK bitmap
	DW 128                        ; CKS: (DRM+1)/4, zero for non-removable media
...
; Boot message.
MSG_GoCPM:
	DB "56K TPA CP/M 2.2         BIOS BY BITROWZ", LF, CR
